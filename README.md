...
- 자체 실측 진동 데이터: **없음**
- **공개 실데이터(CWRU, MaFaulDa) 검증: 완료** — 시뮬레이션으로 설계한 파이프라인을
  실제 회전기계 데이터로 검증하고 고장 분류 ML 모델까지 학습함. [`ml/README.md`](ml/README.md) 참고.
...
- 자체 실측(우리 하드웨어) 성능은 아직 검증되지 않았습니다. 공개 실데이터(CWRU,
  MaFaulDa) 기준 성능은 `ml/README.md`에 정리돼 있으나, 실험실 장비로 측정된
  데이터이므로 저가 센서(MPU-6050) 환경의 성능을 보장하지 않습니다.
...
이 저장소는 이미 작동하는 시스템이 아니라 실제 데이터가 들어올 자리를
미리 만들어 둔 설계도
